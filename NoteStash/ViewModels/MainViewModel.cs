...
public partial class MainViewModel : ObservableObject
{
    ...
    /// <param name="aiService">The AI service that will enable interacting with an AI chatbot.</param>
    /// <param name="settings">The settings manager for the application.</param>
    public MainViewModel(IDialogService dialogService, ITextEditorService textEditorService, IAiService aiService, Settings settings)
    {
        ...
        // For setter checking
        _tabs = [2, 4, 6, 8, 10];
        _autosaveIntervals = [500, 1000, 2000, 3000, 4000, 0];
    }

    #region AI Commands

    private async Task AiTextCommand(Func<string, Task<string>> fn)
    {
        bool selectionValid = !string.IsNullOrEmpty(SelectedText);
        string textToExpand = selectionValid ? SelectedText : InputText;
        string text = await fn(textToExpand);

        if (!string.IsNullOrEmpty(text))
        {
            if (text.StartsWith("ERROR"))
            {
                ShowAndFadeInfoPopup(text);
                return;
            }

            if (selectionValid)
            {
                _textEditorService.SetSelectedText(text);
            }
            else
            {
                InputText = text;
    ...
    [RelayCommand]
    [property: ExecutableAction("Expand on Text", "", "")]
    private async Task ExpandOnText() => await AiTextCommand(_aiService.ExpandOnText);

    [RelayCommand]
    [property: ExecutableAction("Summarize Text", "", "")]
    private async Task SummarizeText() => await AiTextCommand(_aiService.SummarizeText);

    [RelayCommand]
    [property: ExecutableAction("Fix Grammar", "", "")]
    private async Task FixGrammar() => await AiTextCommand(_aiService.FixGrammar);

    [RelayCommand]
    [property: ExecutableAction("Explain Text", "", "")]
    private async Task ExplainText() => await AiTextCommand(_aiService.ExplainText);
    ...
    private void CloseInfoPopup()
    {
        ...
        InfoPopupText = string.Empty;
    }

    [ObservableProperty]
    private float _infoPopupOpacity;

    [ObservableProperty]
    private string _infoPopupText;

    [ObservableProperty]
    private bool _infoPopupVisible;

    private async void ShowAndFadeInfoPopup(string text)
    {
        InfoPopupOpacity = 1;
